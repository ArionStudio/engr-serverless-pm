...
supported by the current entry contract. Device/tag/configuration read workflows
also need explicit contracts; none of these gaps should be filled by React reading
...
character classes. Vault display names are generated by core. Missing recovery,
settings and read contracts remain integration gaps; gallery fixtures allow the
